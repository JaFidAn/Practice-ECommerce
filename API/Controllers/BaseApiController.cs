...
public class BaseApiController : ControllerBase
{
    ...
    protected async Task<ActionResult> CreatePagedResult<T, TDto>(IGenericRepository<T> repository, ISpecification<T> spec, int pageIndex, int pageSize, Func<T, TDto> toDto) where T : BaseEntity, IDtoConvertable
    {
        var items = await repository.GetAsync(spec);
        var count = await repository.CountAsync(spec);
